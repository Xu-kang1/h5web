<template>
    <div class="bg">
        <mt-header fixed title="每日新品" v-if="showHeader">
            <router-link to="/" tag='li' slot="left">
                <!-- <mt-button icon="back"></mt-button> -->
            </router-link>
        </mt-header>
        <div v-if="showHeader" class="left_icon" @click="goBack"><span></span></div>
        <div class="box" ref='viewBox'>
            <div v-for="(list, index) in lists" :key="list.id" class="list">
                ...
                <div class="line"></div>
                <div class="note">{{list.description}}</div>
                <div @click="goApply(index,list.productConfigureType)" class="btn">立即借款</div>
            </div>
        </div>
        <div class="btnNav">
            <div class="imgBox">
...
export default {
    ...
    methods: {
        ...
        // 统计产品点击 立即借款跳转
        goApply (index,productConfigureType) {
        ...
        checkStatus (index,productConfigureType) {
            if(productConfigureType == 1 && this.userId !=undefined){	//是否联合登录产品
...
.bg{
    width:100%;
    min-height:80%;
    background-image:url('../../../static/999.png');
